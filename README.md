...
| 表示モード切替 | `landscape` / `portrait` / `minimized` の3モードを切り替え可能。各モードのサイズと表示位置を個別に保持 |
| ラベル管理 | 作業ディレクトリごとにラベル名・色を保存。`landscape` の Label 列と `portrait` の `+ Label` ボタンから編集可能 |
...
| 1秒間隔の自動更新 | `landscape` / `portrait` では1秒ごとに自動更新。`minimized` 中は停止し、`Restore` 時に即時更新 |
...
| Label | ラベル名。未設定時は `+ Label`、ディレクトリ未取得時は `No Label` を表示（`landscape` モードのみ） |
...
| 操作 | 動作 |
|------|------|
| `Portrait` / `Wide` ボタン | `landscape` と `portrait` を切り替え。各モードのサイズと表示位置は個別に保存 |
| `Minimize` ボタン | `minimized` モードに切り替え。`Restore` ボタンだけを表示する小さな画面になる |
| `Restore` ボタン | `minimized` から直前の表示モードと表示位置に戻り、即時リフレッシュ |
| Label列（`landscape`） | Labelセルをクリックしてラベルを追加・編集 |
| `+ Label` ボタン（`portrait`） | カード上の `+ Label` をクリックしてラベルを追加・編集 |
...
| 最後に使った通常表示モード | `settings.json` (`layout_mode`) |
| 各表示モードのウィンドウサイズと表示位置 | `settings.json` (`window_geometries.landscape` / `portrait` / `minimized`) |
