...
| 表示モード切替 | `Wide` (横長のリスト) 表示、`Portrait` (縦長のカード) 表示、`Minimize` (最小化) 表示を切り替え可能。各表示のサイズと位置を個別に保持 |
...
| 1秒間隔の自動更新 | `Wide` / `Portrait` 表示では1秒ごとに自動更新。`Minimize` 表示では停止し、`Restore` 時に即時更新 |
...
| Label | ラベル名。`Wide` 表示では Label 項目に表示され、未設定時は `+ Label`、ディレクトリ未取得時は `No Label` を表示。`Portrait` 表示ではカード上の `+ Label` ボタンまたは保存済みラベルとして表示 |
...
| 各表示モードのウィンドウサイズと表示位置（`Wide` / `Portrait` / `Minimize`） | `settings.json` (`window_geometries.landscape` / `portrait` / `minimized`) |
