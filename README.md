...
| 2秒間隔の自動更新 | `Table` / `Cards` 表示では2秒ごとに自動更新。`Minimize` 表示では停止し、`Restore` 時に即時更新 |
...
`scripts\windows\launch_ai_cli_watcher.bat` をダブルクリックします。
...
| `Auto refresh: 2s` 表示 | 2秒ごとにプロセス一覧を自動更新 |
...
```
AI-CLI-Watcher/
├── ai_cli_watcher.py        # メインアプリケーション
├── requirements.txt         # 依存パッケージ (psutil)
├── settings.json            # ユーザー設定（自動生成）
├── .gitignore
├── scripts/
│   └── windows/
│       └── launch_ai_cli_watcher.bat   # 起動用バッチファイル
└── README.md                # このファイル
```
